...
def missing_rows(tsdata, pwdata):
    tsseq = []
    for data in tsdata:
        #print data
        tsseq.append(data[1])
    ...
    return uniqts, uniqpw, cols

def mismatch_values(max_raws, uniqts, uniqpw, tsdata, pwdata, cols):
    n=1
    c=1
    while n <= max_raws:
...
(uniqts, uniqpw, cols) = missing_rows(tsdata, pwdata)
mismatch_values(max_raws, uniqts, uniqpw, tsdata, pwdata, cols)
